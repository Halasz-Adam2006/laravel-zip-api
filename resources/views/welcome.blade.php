...
<head>
    ...
    <title>{{ config('app.name', 'Laravel ZIP API') }}</title>
...
<body class="min-h-screen bg-slate-950 text-slate-100">
    <div class="relative overflow-hidden">
        <div class="absolute inset-0 bg-gradient-to-br from-indigo-500/15 via-slate-900/40 to-slate-950"></div>
        <div class="absolute -top-32 -right-32 h-80 w-80 rounded-full bg-indigo-500/20 blur-3xl"></div>
        <div class="absolute -bottom-32 -left-32 h-80 w-80 rounded-full bg-blue-500/20 blur-3xl"></div>

        <main class="relative mx-auto flex min-h-screen max-w-6xl flex-col gap-12 px-6 py-16">
            <header class="flex flex-col gap-4">
                <span
                    class="inline-flex w-fit items-center gap-2 rounded-full border border-indigo-500/40 bg-indigo-500/10 px-4 py-1 text-xs font-semibold uppercase tracking-[0.3em] text-indigo-200">
                    Fullstack
                </span>
                <h1 class="text-4xl font-semibold leading-tight text-white sm:text-5xl">
                    Hungarian postal code lookup
                    <span class="block text-indigo-300">Laravel API + Tailwind frontend</span>
                </h1>
                <p class="max-w-2xl text-base text-slate-300 sm:text-lg">
                    Search by postal code, place name, or county. Results stream live from your Laravel backend.
                </p>
            </header>

            <section
                class="grid gap-6 rounded-3xl border border-white/10 bg-white/5 p-6 shadow-2xl backdrop-blur sm:p-10">
                <form id="postal-search-form" class="flex flex-col gap-4 sm:flex-row sm:items-center">
                    <div class="flex-1">
                        <label for="postal-search-input" class="mb-2 block text-sm font-medium text-slate-200">
                            Search query
                        </label>
                        <input id="postal-search-input" name="query" type="text" placeholder="e.g. 1051, Budapest, Pest"
                            class="w-full rounded-2xl border border-white/10 bg-slate-900/70 px-4 py-3 text-base text-white shadow-inner shadow-black/20 outline-none ring-indigo-400/40 transition focus:border-indigo-400/70 focus:ring-2"
                            autocomplete="off" />
                    </div>
                    <button type="submit"
                        class="inline-flex items-center justify-center rounded-2xl bg-indigo-500 px-6 py-3 text-sm font-semibold text-white transition hover:bg-indigo-400">
                        Search
                    </button>
                </form>
                ...
                <div class="flex flex-wrap gap-2 text-xs text-slate-300">
                    <button type="button"
                        class="example-chip rounded-full border border-white/10 bg-white/5 px-3 py-1 text-xs font-medium text-slate-200 transition hover:border-indigo-400/60 hover:text-white"
                        data-example="1051">1051</button>
                    <button type="button"
                        class="example-chip rounded-full border border-white/10 bg-white/5 px-3 py-1 text-xs font-medium text-slate-200 transition hover:border-indigo-400/60 hover:text-white"
                        data-example="Budapest">Budapest</button>
                    <button type="button"
                        class="example-chip rounded-full border border-white/10 bg-white/5 px-3 py-1 text-xs font-medium text-slate-200 transition hover:border-indigo-400/60 hover:text-white"
                        data-example="Baranya">Baranya</button>
                    <button type="button"
                        class="example-chip rounded-full border border-white/10 bg-white/5 px-3 py-1 text-xs font-medium text-slate-200 transition hover:border-indigo-400/60 hover:text-white"
                        data-example="Debrecen">Debrecen</button>
                </div>

                <div class="flex items-center justify-between text-xs text-slate-400">
                    <span id="postal-search-status">Start typing to search.</span>
                    <span class="hidden sm:inline">Endpoint: <span
                            class="font-mono text-slate-200">/api/postal-codes</span></span>
                </div>
                    ...
                    <table class="min-w-full text-left text-sm">
                        <thead class="bg-slate-900/80 text-xs uppercase text-slate-400">
                            <tr>
                                ...
                                <th class="px-4 py-3">Place name</th>
                                <th class="px-4 py-3">County</th>
                            </tr>
                        </thead>
                        <tbody id="postal-search-results" class="divide-y divide-white/5">
                            <tr>
            ...
            <footer class="text-xs text-slate-500">
                Built with Laravel {{ app()->version() }} and Vite. Frontend powered by Tailwind CSS.
            </footer>
        </main>
    </div>
</body>
